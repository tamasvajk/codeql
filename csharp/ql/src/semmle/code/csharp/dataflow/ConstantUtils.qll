...
private predicate constantIntegerExpr(Expr e, int val) {
  ...
  exists(ExplicitDefinition v, Expr src |
    e = v.getARead() and
    src = v.getADefinition().getExpr().(Assignment).getRValue() and
...
private int getAggrArrayLength(ArrayCreation arrCreation, int index) {
  index = 0 and result = getArrayLength(arrCreation, 0)
  or
  index > 0 and
  result = getArrayLength(arrCreation, index) * getAggrArrayLength(arrCreation, index - 1)
}
...
private predicate isArrayLengthAccess(PropertyAccess pa, int length) {
  ...
  exists(ExplicitDefinition arr, ArrayCreation arrCreation |
    getAggrArrayLength(arrCreation, arrCreation.getNumberOfLengthArguments() - 1) = length and
    arrCreation = arr.getADefinition().getExpr().(Assignment).getRValue() and
